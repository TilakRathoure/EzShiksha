...
const EquationSolver = () => {
  ...
  const handlesubmit = async (e) => {
    if (selectedImage) {
      ...
      formData.append("imagesolve", selectedImage);

      try {
        const { data } = await axios.post(`${server}/users/solve`, formData, {
          headers: {
  ...
  return (
    <div className="w-[100vw] flex-col justify-center items-center p-8">
      <div
        className={`relative w-full bg-white rounded-lg shadow-lg p-8 ${
          selectedImage ? "h-auto" : "h-[75vh]"
        }`}
      >
        <a
          href="/Testing/formula_test.png"
          download="formula_test.png"
          className="bg-gray-400 absolute -top-5 right-0 text-white px-2 py-2 rounded-lg no-underline"
        >
              ...
              {selectedImage && (
                <div className="relative">
                  <img
                    ...
                    className="w-full max-h-[50vh] rounded-md"
                    alt="Preview"
                  />
                  <button
